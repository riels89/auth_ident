...
def tokenize(type):
    ...
    command = [
        "./../Project_CodeNet/tools/tokenizer/tokenize", "-lC++", "-mcsv", "-n",
        "-s", "-1", "-a", f"-o{tokenized_path}"
    ]
...
if not args.l and not args.le:
    ...
    data = pd.read_csv(tokenized_train_path, encoding="latin1",
            error_bad_lines=True, header=None, names=["class", "token"])
    tokens = data[data['class'] == "identifier"]["token"].to_numpy(
        dtype="U20")
...
def encode(type):
    if not args.le:
        subprocess.call([
            "auth_ident/preprocessing/encode_tokens", f"-n{num_reserved_identifiers}",
            path + f"_{type }"
        ])
